...
const (
	...
	testRegisterTime = "2026-01-01T00:00:00Z"
	testProfileTime  = "2026-03-15T12:30:00Z"
	testLangEn       = "en"
	testLangRu       = "ru"
)
...
func TestNewRegisterResponse(t *testing.T) {
	...
	assertUserResponseFields(t, &userResponseFields{
		...
		email:     user.Email,
		createdAt: testRegisterTime,
		updatedAt: testRegisterTime,
	}, resp)
}
...
func TestNewTranslationHistory(t *testing.T) {
	...
	history := entity.TranslationHistory{
		History: []entity.Translation{
			{
				Source:      testLangEn,
				Destination: testLangRu,
				Original:    "hello",
				Translation: "привет",
			},
			{
				Source:      testLangRu,
				Destination: testLangEn,
				Original:    "мир",
				Translation: "world",
			},
		},
	...
	assert.Equal(t, testLangEn, resp.History[0].Source)
	assert.Equal(t, testLangRu, resp.History[0].Destination)
	assert.Equal(t, "hello", resp.History[0].Original)
	assert.Equal(t, "привет", resp.History[0].Translation)
	assert.Equal(t, testLangRu, resp.History[1].Source)
	assert.Equal(t, testLangEn, resp.History[1].Destination)
