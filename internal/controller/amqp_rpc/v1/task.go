...
type transitionTaskData struct {
	ID     string `json:"id"`
	Status string `json:"status"`
}
...
func (r *V1) createTask() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - createTask - invalid token: %w", err)
		}
...
func (r *V1) getTask() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		...
		userID, err := r.j.ParseToken(req.Token)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - getTask - invalid token: %w", err)
		}
...
func (r *V1) listTasks() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		...
		userID, err := r.j.ParseToken(req.Token)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - listTasks - invalid token: %w", err)
		}
...
func (r *V1) updateTask() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		...
		userID, err := r.j.ParseToken(req.Token)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - updateTask - invalid token: %w", err)
		}
...
func (r *V1) transitionTask() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		var req struct {
			Token string             `json:"token"`
			Data  transitionTaskData `json:"data"`
		}
		...
		userID, err := r.j.ParseToken(req.Token)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - transitionTask - invalid token: %w", err)
		}
...
func (r *V1) deleteTask() server.CallHandler {
	return func(d *amqp.Delivery) (any, error) {
		...
		userID, err := r.j.ParseToken(req.Token)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - deleteTask - invalid token: %w", err)
		}
